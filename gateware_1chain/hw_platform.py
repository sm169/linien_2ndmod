...
from migen.build.xilinx import XilinxPlatform, vivado
from migen import *
...
class DummyPlatform:
    ...
    def add_period_constraint(self, *args):
        pass

    def add_false_path_constraint(self, *args):
        pass

    def add_platform_command(self, *args):
        pass
#use yosys for rtl elabotated.v and no synth

'''from migen.build import tools

class CustomVivadoToolchain(vivado.XilinxVivadoToolchain):
    def __init__(self):
        super().__init__()
    def _build_batch(self, platform, sources, edifs, ips, build_name):
        ...
        for filename, language, library in sources:
            tcl.append("add_files -norecurse {}".format(filename))
            tcl.append("set_property library {} [get_files {}]".format(library, filename))
        tcl.append("read_xdc {}.xdc".format(build_name))
        tcl.append("synth_design -rtl -top {} -part {}".format(build_name, platform.device))
        tcl.append("write_verilog -force {}_elaborated.v".format(build_name))
        tcl.append("quit")
...
class CustomPlatform(Platform):
    def __init__(self):
        super().__init__()
        self.toolchain = CustomVivadoToolchain()'''
